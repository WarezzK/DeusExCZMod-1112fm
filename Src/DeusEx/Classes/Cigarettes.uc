...
function SetSkin()
{
	...
	if(lSkin != None)
	{
		...
		if (Cig == SC_BigTop)
		{
			Description = "Big Top Cigarettes -- Elephant tested, Lion Approved!";
			icon 		= Texture(DynamicLoadObject("HDTPitems.skins.belticonCigarettes2", Class'Texture', True));
			largeicon   = Texture(DynamicLoadObject("HDTPitems.skins.largeiconCigarettes2", Class'Texture', True));
		}
	}
}
	...
	function BeginState()
	{
		...
		if (P != None)
		{
			...
			if (puff != None)
			{
				puff.DrawScale = 1.0;
				//if(Frand() >= 0.5)
					//puff.DamageType = 'CigSmoke';
				puff.Velocity = vect(0,0,0);
			}
