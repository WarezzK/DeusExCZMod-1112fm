...
function CreateAccountsList()
{
	...
	lstAccounts.SetColumnWidth(0, 105);
	lstAccounts.SetColumnWidth(1, 65);
	lstAccounts.SetColumnColor(1, colUnhackable);
	lstAccounts.SetColumnAlignment(1, HALIGN_RIGHT);
}
...
function SetCompOwner(ElectronicDevices newCompOwner)
{
	...
	for (compIndex=0; compIndex<compOwner.NumUsers(); compIndex++)
	{
		...
		if (compOwner.GetAccessLevel(compIndex) > 0)
		{
			if (compOwner.GetAccessLevel(compIndex) > player.SkillSystem.getSkillLevel(class'SkillComputer') && !player.HasPass(compOwner.GetPassword(compIndex), compOwner.GetUserName(compIndex)))
			{
				lstAccounts.SetRowClientInt(lstAccounts.IndexToRowId(compIndex), -1);
				
				lstAccounts.SetField(lstAccounts.IndexToRowId(compIndex), 1, "[" $ player.SkillSystem.GetSkillFromClass(Class'SkillComputer').skillLevelStrings[compOwner.GetAccessLevel(compIndex)] $ "]");
			}
		}
	}
...
function ChangeSelectedAccount()
{
	...
	if (lstAccounts.GetRowClientInt(lstAccounts.GetSelectedRow()) == -1)
	{
		player.PlaySound(Sound'DeusExSounds.Generic.Buzz1', SLOT_None);
		return;
	}
