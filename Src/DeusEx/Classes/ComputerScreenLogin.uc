...
function SetNetworkTerminal(NetworkTerminal newTerm)
{
	...
	if (winTerm != None)
	{
		winTerm.CreateHackWindow();
		winTerm.SetHackButtonToReturn();
		winTerm.CreateAvailAccountsWindow(Self);
	}
}
...
function SetLoginText(string userName, string password)
{
...
function ProcessLogin()
{
	...
	for (compIndex=0; compIndex<Computers(compOwner).NumUsers(); compIndex++)
	{
		...
		{
			userName  = Computers(compOwner).GetUserName(compIndex);
			userIndex = compIndex;
			break;
		}
	}
	...
	if (userIndex != -1)
	{
		if (Caps(editPassword.GetText()) == Caps(Computers(compOwner).GetPassword(userIndex)))
		{
			...
			player.AddPass(editPassword.GetText(), editUserName.GetText());
		}
	}
	...
	if (bSuccessfulLogin)
	{
		...
		if (winTerm.GetSkillLevel() < userSkillLevel)
			...
			winTerm.CreateAvailAccountsWindow(None);
