...
function SelectAugmentation(PersonaItemButton buttonPressed)
{
	...
	if (selectedAugButton != buttonPressed)
	{
		...
		else if (HUDMedBotAugItemButton(buttonPressed).bHasIt)
		{
			winInfo.Clear();
			winInfo.SetTitle(selectedAug.AugmentationName);
			winInfo.SetText(AlreadyHasItText);
			if(selectedAug.CurrentLevel < selectedAug.MaxLevel || (selectedAug.bBoosted && selectedAug.CurrentLevel <= selectedAug.MaxLevel))
			{
...
function InstallAugmentation(optional int level)
{
	...
	//a: the above doesn't actually destroy the item
	if (augCan != None)
	augCan.Destroy();
...
defaultproperties
{
     ...
     NoCansAvailableText="No Augmentation Canisters Available!"
