...
simulated function PreBeginPlay()
{
	...
	if (player != None && player.NanoMod != None && player.NanoMod.doubleCarryingCap)
	MaxAmmo = Default.MaxAmmo * 2;

	if(Level.NetMode == NM_StandAlone && player != None)
	{
		if (player.NanoMod.hasHDTP || IsA('Ammo10mmEX') || IsA('AmmoDragon'))
		Facelift(true);
	}
}
...
function bool AddAmmo(int AmmoToAdd)
{
	...
	if(isGrenade)
	{
		demoSkill = DeusExPlayer(GetPlayerPawn()).SkillSystem.GetSkillLevel(class'SkillDemolition') * (MaxAmmo / 3);
