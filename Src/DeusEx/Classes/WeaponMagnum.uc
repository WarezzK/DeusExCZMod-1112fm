...
function bool Facelift(bool bOn)
{
	...
	if(!Super.Facelift(false))
		return false;

	if(bOn)
	{
		temptex = Texture(DynamicLoadObject("ShifterEX.Icons.LargeIconMagnum",class'Texture', True));
		if(temptex != None)
			largeIcon = temptex;

		temptex = Texture(DynamicLoadObject("ShifterEX.Icons.BeltIconMagnum",class'Texture', True));
		if(temptex != None)
			Icon = temptex;
