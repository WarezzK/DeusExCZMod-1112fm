...
{
	local int i, lowestLevel;
	local Computers compOwner;
	local Color colUnhackable;
	
	compOwner = Computers(newCompOwner);
	
	colUnhackable   = colRed;
	colUnhackable.R = 128;
	
	if (compOwner.NumUsers() == 0)
	lowestLevel = 0;
	...
	else
	{
		...
		for (i = 0; i < compOwner.NumUsers(); i++)
		{
			if (lowestLevel > compOwner.userList[i].accessLevel)
			lowestLevel = compOwner.userList[i].accessLevel;
		}
	}
	
	if (player.SkillSystem.getSkillLevel(class'SkillComputer') < lowestLevel)
	{
		bUnhackable = True;
		
		winHackMessage.SetTextColor(colUnhackable);
		SetHackMessage(player.SkillSystem.GetSkillFromClass(Class'SkillComputer').skillLevelStrings[lowestLevel] @ "SKILL LEVEL REQUIRED");
	}
}
