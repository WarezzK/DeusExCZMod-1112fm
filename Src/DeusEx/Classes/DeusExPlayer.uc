...
// --
// alex: Well, from My experience -- just don't mess with the order of the travel vars.
// i.e., do not insert any travel vars inbetween existing ones, and do not modify their order.
...
/*
1 = Ship with 1, 2 = Extended, 3 = Personal;;; Extended not used.
...
var travel String availPass[256];
var travel bool noMoreAccuracyMods;
var travel bool noMoreClipMods;
var travel bool noMoreRangeMods;
var travel bool noMoreReloadMods;
var travel bool noMoreRecoilMods;
var travel NanoModSettings NanoMod;
...
function InitializeSubSystems()
{
	...
	if (NanoMod == None)
		NanoMod = Spawn(class'NanoModSettings', Self);
		
	if (NanoMod.p != Self)
	NanoMod.SetPlayer(Self);
...
function PostPostBeginPlay()
{
	...
	//a: only fires on old saves and when travelling to old maps
	if (NanoMod == None)
	{
		NanoMod = Spawn(class'NanoModSettings', Self);
		NanoMod.SetPlayer(Self);
...
event TravelPostAccept()
{
	...
	if (NanoMod != None && (NanoMod.p != Self || !flagBase.GetBool('PlayerTraveling')))
	NanoMod.SetPlayer(Self);
...
{
	local Actor generic;
	local WeaponWPGrenade weapWp;
	local WeaponMagnum weapMag;
	local Ammo10mmEX   ammoEX;
	local AmmoDragon   ammoDr;
	local WPGrenade projWp;
	local WeaponModAuto modAuto;
	local SignFloor floorSign;
	local Lamp dLamp;
	...
	if (!NanoMod.hasHDTP)
	{
		foreach AllActors(class'WeaponWPGrenade', weapWp)
			weapWp.Facelift(bOn);
			
		foreach AllActors(class'WeaponMagnum', weapMag)
			weapMag.Facelift(bOn);
			
		foreach AllActors(class'Ammo10mmEX', ammoEX)
			ammoEX.Facelift(bOn);
			
		foreach AllActors(class'AmmoDragon', ammoDr)
			ammoDr.Facelift(bOn);
		
		foreach AllActors(class'WPGrenade', projWp)
			projWp.Facelift(bOn);
			
		foreach AllActors(class'WeaponModAuto', modAuto)
			modAuto.Facelift(bOn);
			
		foreach AllActors(class'SignFloor', floorSign)
			floorSign.Facelift(bOn);
			
		foreach AllActors(class'Lamp', dLamp)
			dLamp.Facelift(false);
	...
	Facelift((bOn && NanoMod.enableHDTPHumans)); // Make the player's model look all fancy-like
	...
	foreach AllActors(class'Actor', generic)
	{
		...
		{
			if (WeaponWPGrenade(generic) != None || WeaponMagnum(generic) != None)
			DeusExWeapon(generic).Facelift(bOn);
			
			else
			DeusExWeapon(generic).Facelift((bOn && !NanoMod.disableHDTPWeapons));
		}
		...
		{
			if (Robot(generic) != None || Animal(generic) != None)
			ScriptedPawn(generic).Facelift(bOn);
			
			else
			ScriptedPawn(generic).Facelift((bOn && NanoMod.enableHDTPHumans));
		}

		if(DeusExCarcass(generic) != None)
		{
			if (DeusExCarcass(generic).bAnimalCarcass)
			DeusExCarcass(generic).Facelift(bOn);
			
			else
			DeusExCarcass(generic).Facelift((bOn && NanoMod.enableHDTPHumans));
		}

		if(DeusExProjectile(generic) != None)
		{
			if (WPGrenade(generic) != None)
			DeusExProjectile(generic).Facelift(bOn);

			else
			DeusExProjectile(generic).Facelift((bOn && !NanoMod.disableHDTPWeapons));
		}
...
function ResetPlayerToDefaults()
{
   ...
   // reset the image linked list
	...
	if (versionType == 3)
	{
		...
		Default.SkillPointsTotal = 4425;
		Default.SkillPointsAvail = 4425;
		//Default.Energy    = 1000;
		//Default.EnergyMax = 1000;
	}
   ...
   for (i = 0; i<256; i++)
	...
	noMoreAccuracyMods = False;
	noMoreClipMods     = False;
	noMoreRangeMods	   = False;
	noMoreReloadMods   = False;
	noMoreRecoilMods   = False;
}
		...
		if ((Velocity.Z < -700) && (ReducedDamageType != 'All'))
			if ( Role == ROLE_Authority )
            {
				...
				if (AugmentationSystem != None)
				{
					...
					if(Int(SkillSystem.getSkillLevel(class'SkillSwimming')) == 3)
					augReduce += 15;
				}
...
function DoJump( optional float F )
{
	...
	if (Physics == PHYS_Walking || Physics == PHYS_Spider)
	{
		...
		if (augLevel > 1.0)
		Velocity.Z += Default.JumpZ * (augLevel - 1.0);
...
function float GetCurrentGroundSpeed()
{
   ...
   augValue = AugmentationSystem.GetAugLevelValue(class'AugSpeed');
	...
	if(!bIsCrouching && !bForceDuck)
	{
		...
		if (augValue > 1.0)
		speed += Default.GroundSpeed * (augValue - 1.0);
	...
	else 
	{
		...
		if (foundItem != None)
		{
			...
			{ //Justice: A pain in the ass for them, but a blessing for me
				if (DeusExWeapon(foundItem).AmmoType.AmmoAmount >= DeusExWeapon(foundItem).AmmoType.MaxAmmo + (DeusExWeapon(foundItem).AmmoType.MaxAmmo / 3) * SkillSystem.GetSkillLevel(class'SkillDemolition'))
				{
					ClientMessage(TooMuchAmmo);
					bCanPickup = False;
				}
...
//					ClientMessage(Sprintf(CanCarryOnlyOne, foundItem.itemName));
				...
				if (DeusExWeapon(FindInventoryType(foundItem.Class)) != None)
				{
					if (DeusExWeapon(FrobTarget).PickupAmmoCount > 0 && DeusExWeapon(foundItem).AmmoType.AmmoAmount >= DeusExWeapon(foundItem).AmmoType.MaxAmmo)
					{
						ClientMessage(TooMuchAmmo);
						bCanPickup = False;
					}
...
// FixNanoKey()
//
// This is called on game load, and on key pickup.
// This will normalize all nanokey names in the game and remove unused
// keys.
//
// On pickup this will deny unused keys.
//
// On game load this will fix the key list from a vanillia save,
// or rename fixed keys in case I want to rename some of them later.
//
// 																 -- alex
// ----------------------------------------------------------------------

function NanoKey FixNanoKey(NanoKey newKey)
{
	     if (newKey.KeyID == 'UNOfficeDoorKey')				{ newKey.Description = "UNATCO HQ Office"; }
	else if (newKey.KeyID == 'UNcloset')        		 	{ newKey.Description = "UNATCO HQ Storage Closet"; }
	else if (newKey.KeyID == 'JaimeClosetKey') 			 	{ newKey.Description = "UNATCO HQ MedLab Closet"; }
	else if (newKey.KeyID == 'UNhatchdoor')  			 	{ newKey.Description = "UNATCO HQ Compound Hatch"; }
	else if (newKey.KeyID == 'ruin_key')  				 	{ newKey.Description = "Island Statue Ruin"; }
	else if (newKey.KeyID == 'KioskDoors')  			 	{ newKey.Description = "Castle Clinton Kiosk"; }
	else if (newKey.KeyID == 'CCSafe')  				 	{ newKey.Description = "Castle Clinton Kiosk Safe"; }
	else if (newKey.KeyID == 'CrackRoom')  				 	{ newKey.Description = "'Ton Hotel North Room"; }
	else if (newKey.KeyID == 'SewerKey')  				 	{ newKey.Description = "Hell's Kitchen Sewers"; }
	else if (newKey.KeyID == 'Apartment')				    { newKey.Description = "'Ton Hotel, Paul's Apartment"; }
	else if (newKey.KeyID == 'StreetWarehouse') 		 	{ newKey.Description = "Osgood and Sons' Front Door"; }
	else if (newKey.KeyID == 'WarehouseAccess') 		 	{ newKey.Description = "Osgood and Sons' Back Door"; }
	else if (newKey.KeyID == 'WarehouseBasementStorage') 	{ newKey.Description = "Warehouse Basement Storeroom"; }
	else if (newKey.KeyID == 'WarehouseFrontDoor') 		 	{ newKey.Description = "Warehouse Front Door"; }
	else if (newKey.KeyID == 'Mirror') 		 			 	{ newKey.Description = "Warehouse Secret Mirror Door"; }
	else if (newKey.KeyID == 'lebedevdoor') 		 	 	{ newKey.Description = "747, Lebedev's Private Quarters"; }
	else if (newKey.KeyID == 'eastgate') 		 	 	 	{ newKey.Description = "Airfield East Gate"; }
	else if (newKey.KeyID == 'Dormitory') 		 	 	 	{ newKey.Description = "Airfield Barracks Dormitory"; }
	else if (newKey.KeyID == 'securitytower') 		 	 	{ newKey.Description = "Airfield Security Tower"; }
	else if (newKey.KeyID == 'HeliSecurityRoom') 		 	{ newKey.Description = "Helibase Security Room"; }
	else if (newKey.KeyID == 'Sewerdoor') 		 		 	{ newKey.Description = "Helibase Sewer Door"; }
	else if (newKey.KeyID == 'MoleRestroomKey') 		 	{ newKey.Description = "Mole People Bathroom"; }
	else if (newKey.KeyID == 'ManderleyOfficeDoor') 	 	{ newKey.Description = "UNATCO HQ, Manderley's Office"; }
	else if (newKey.KeyID == 'BasementDoor') 	 		 	{ newKey.Description = "NSF HQ Basement Door"; }
	else if (newKey.Description == "MIB Agent Cabinet Key") { newKey.KeyID = 'MIBCabinet'; }
	else if (newKey.KeyID == 'levelone') 	 		 	 	{ newKey.Description = "UNATCO HQ Front Door"; }
	else if (newKey.KeyID == 'control_door01') 	 		 	{ newKey.Description = "MJ12 Helibase Flight Control Deck"; }
	else if (newKey.KeyID == 'LockerMasterKey') 	 		{ newKey.Description = "MJ12 Helibase Barracks Locker"; }
	else if (newKey.KeyID == 'Ordinance_bay') 	 			{ newKey.Description = "MJ12 Helibase Munitions Bay"; }
	else if (newKey.KeyID == 'SubjectDoors') 	 			{ newKey.Description = "MJ12 Lab Experimental Subject Cell"; }
	else if (newKey.KeyID == 'Magnetic_Chamber') 	 		{ newKey.Description = "MJ12 Lab Magnetic Augmentation Chamber"; }
	else if (newKey.KeyID == 'NanoContainmentDoor') 	 	{ newKey.Description = "Nanotech Containment Lockdown Door"; }
	else if (newKey.KeyID == 'ncHarrison') 	 				{ newKey.Description = "Nanotech Containment, Harrison's Cabinet"; }
	else if (newKey.KeyID == 'JocksKey') 	 			    { newKey.Description = "Tonnochi Road, Jock's Apartment"; }
	else if (newKey.KeyID == 'CompoundGate') 	 			{ newKey.Description = "Luminous Path Compound"; }
	else if (newKey.KeyID == 'WeaponWarehouse') 	 		{ newKey.Description = "Shipyard Ammo Storage Warehouse"; }
	else if (newKey.Description == "Key to Supply Closet")  { newKey.KeyID = 'SupplyRoom'; }
	else if (newKey.KeyID == 'CommandersOffice') 	 		{ newKey.Description = "Shipyard Commander's Office"; }
	else if (newKey.KeyID == 'WeaponLocker') 	 			{ newKey.Description = "Shipyard Weapon Storage Closet"; }
	else if (newKey.KeyID == 'SemiKey') 	 				{ newKey.Description = "Shipyard Truck Trailer"; }
	else if (newKey.KeyID == 'MainGate') 	 				{ newKey.Description = "Shipyard Main Gate"; }
	else if (newKey.KeyID == 'EngineRoomDoor') 	 			{ newKey.Description = "Ship Engine Room"; }
	else if (newKey.KeyID == 'HeliBayDoorLock') 	 		{ newKey.Description = "Lower Decks Heli-Bay Ops Room"; }
	else if (newKey.KeyID == 'Locker1') 	 				{ newKey.Description = "Lower Decks Locker 1"; }
	else if (newKey.KeyID == 'Locker2') 	 				{ newKey.Description = "Lower Decks Locker 2"; }
	else if (newKey.KeyID == 'FanDoor') 	 				{ newKey.Description = "Shipyard Fan Door"; }
	else if (newKey.KeyID == 'ug_warehousedoors') 	 		{ newKey.Description = "Denfert-Rochereau Warehouse"; }
	else if (newKey.KeyID == 'cata_officedoor') 	 		{ newKey.Description = "Denfert-Rochereau Office"; }
	else if (newKey.KeyID == 'catdoor') 	 				{ newKey.Description = "Denfert-Rochereau Catacombs"; }
	else if (newKey.KeyID == 'catacombs_blastdoor') 	 	{ newKey.Description = "Catacombs Blast Door"; }
	else if (newKey.KeyID == 'catacombs_blastdoor02') 	 	{ newKey.Description = "Catacombs Sewers"; }
	else if (newKey.KeyID == 'duclare_chateau') 	 		{ newKey.Description = "DuClare Basement"; }
	else if (newKey.KeyID == 'beth_room') 	 				{ newKey.Description = "DuClare, Beth's Bedroom"; }
	else if (newKey.KeyID == 'nico_room') 	 				{ newKey.Description = "DuClare, Nicolette's Bedroom"; }
	else if (newKey.KeyID == 'Crypt_door') 	 				{ newKey.Description = "DuClare Crypt"; }
	else if (newKey.KeyID == 'club_maindoors') 	 			{ newKey.Description = "Paris Nightclub"; }
	else if (newKey.KeyID == 'hotel_roomdoor') 	 			{ newKey.Description = "Paris Hotel Room"; }
	else if (newKey.KeyID == 'cathedralgatekey') 	 		{ newKey.Description = "Cathedral Main Gate"; }
	else if (newKey.KeyID == 'cath_maindoors') 	 			{ newKey.Description = "Cathedral Tower"; }
	else if (newKey.KeyID == 'morganmirror') 	 			{ newKey.Description = "Everett's Mirror Lock Mechanism"; }
	else if (newKey.KeyID == 'fishtankdoor') 	 			{ newKey.Description = "Everett's Aquarium Hatch"; }
	else if (newKey.KeyID == 'morganhelibaydoor') 	 		{ newKey.Description = "Everett's Helipad Maglock"; }
	else if (newKey.KeyID == 'Tunnel_access1') 	 			{ newKey.Description = "Airforce Base Tunnels"; }
	else if (newKey.KeyID == 'Lab02') 	 					{ newKey.Description = "Airforce Base 3rd Floor Computer Room"; }
	else if (newKey.KeyID == 'garage_entrance') 	 		{ newKey.Description = "Gas Station Garage"; }
	else if (newKey.KeyID == 'pipe') 	 					{ newKey.Description = "Gas Station Sewers"; }
	else if (newKey.KeyID == 'control_room') 	 			{ newKey.Description = "Airforce Base Control Room"; }
	else if (newKey.KeyID == 'maintenancekey') 	 			{ newKey.Description = "Airforce Base Tunnels Maintenance"; }
	else if (newKey.KeyID == 'Glab') 	 					{ newKey.Description = "Ocean Lab Greasel Laboratory"; }
	else if (newKey.Description == "Storage Room Key" ||
			 newKey.Description == "Storage Access Key") 	{ newKey.KeyID = 'OLstorage'; }
	else if (newKey.KeyID == 'crewkey') 	 				{ newKey.Description = "Ocean Lab Crew Module"; }
	else if (newKey.KeyID == 'truck') 	 					{ newKey.Description = "Missile Silo Truck Trailer"; }
	else if (newKey.KeyID == 'woodgate') 	 				{ newKey.Description = "Missile Silo Front Gate"; }
	else if (newKey.KeyID == 'Sub_base') 	 				{ newKey.Description = "Sub Base Command Module"; }
	else if (newKey.KeyID == 'shed') 	 					{ newKey.Description = "Sub Base Supply Shed"; }
	else if (newKey.KeyID == 'tower') 	 					{ newKey.Description = "Area 51 Security Tower"; }
	else if (newKey.KeyID == 'door_lower') 	 				{ newKey.Description = "Area 51 Sector 3 Stairwell"; }
	else if (newKey.KeyID == 'Factory') 	 				{ newKey.Description = "Area 51 Sector 3"; }
	else if (newKey.KeyID == 'UN_maindoor'    ||
			 newKey.KeyID == 'ApartmentKey'   ||
			 newKey.KeyID == 'LebedevBedroom' ||
			 newKey.KeyID == 'UnderArea'      ||
			 newKey.KeyID == 'BelowDecks'     ||
			 newKey.KeyID == 'Access'         ||
			 newKey.KeyID == 'None')  						{ newKey.KeyID = ''; }
			 
	if (newKey.KeyID != '')
	{
			 if (newKey.KeyID == 'Cabinet')    { newKey.Description = "Free Clinic Medical Cabinet"; }
		else if (newKey.KeyID == 'MIBCabinet') { newKey.Description = "MJ12 Facility, MIB's Cabinet"; }
		else if (newKey.KeyID == 'SupplyRoom') { newKey.Description = "Shipyard Supply Closet"; }
		else if (newKey.KeyID == 'OLstorage')  { newKey.Description = "Ocean Lab Storage Closet"; }
		else if (newKey.KeyID == 'storage')    { newKey.Description = "Area 51 Storage Vault"; }
	}
...
function bool PickupNanoKey(NanoKey newKey)
{
	...
	{
		ClientMessage("You already have the" @ newKey.Description @ "NanoKey");
...
exec function bool DropItem(optional Inventory inv, optional bool bDrop)
{
		...
		if ((FrobTarget != None) && !item.IsA('POVCorpse'))
		{
			if(ScriptedPawn(FrobTarget) != None && DeusExWeapon(item) != None)
			{
				...
				if(ScriptedPawn(FrobTarget).bCanGiveWeapon && VSize(FrobTarget.Location - Location) <= 64 && ScriptedPawn(FrobTarget).CheckPawnAllianceType(Self) != ALLIANCE_Hostile)
				{
					...
					amItem = None;
					gItem  = ScriptedPawn(FrobTarget).FindInventoryType(item.Class);
					
					if (gItem == None)
					{
						gItem = spawn(item.Class, FrobTarget);
						gItem.InitialState='Idle2';
						gItem.GiveTo(ScriptedPawn(FrobTarget));
						gItem.SetBase(FrobTarget);
					}
					...
					if(Weapon(gItem).AmmoName != Class'AmmoNone')
					{
						if (gItem.IsA('WeaponGrenade') || gItem.IsA('WeaponShuriken') || gItem.IsA('WeaponCombatKnife'))
						{
							amItem = Ammo(FindInventoryType(Weapon(gItem).AmmoName));
							
							if (amItem != None && amItem.AmmoAmount <= 0)
							amItem.AmmoAmount = 1;
						}
						
						Weapon(gItem).AmmoType = Ammo(ScriptedPawn(FrobTarget).FindInventoryType(Weapon(gItem).AmmoName));
						
						if (Weapon(gItem).AmmoType == None)
						{
							Weapon(gItem).AmmoType = spawn(Weapon(gItem).AmmoName,FrobTarget);
							Weapon(gItem).AmmoType.InitialState='Idle2';
							Weapon(gItem).AmmoType.GiveTo(ScriptedPawn(FrobTarget));
							Weapon(gItem).AmmoType.SetBase(FrobTarget);
							
							if (amItem != None)
							Weapon(gItem).AmmoType.AmmoAmount = amItem.AmmoAmount;
							
							else
							Weapon(gItem).AmmoType.AmmoAmount = Weapon(gItem).AmmoType.Default.AmmoAmount * 4;
						}
						
						else
						{
							if (amItem != None)
							Weapon(gItem).AmmoType.AddAmmo(amItem.AmmoAmount);
							
							else
							Weapon(gItem).AmmoType.AddAmmo(Weapon(gItem).AmmoType.Default.AmmoAmount * 4);
						}
					...
					ScriptedPawn(FrobTarget).SwitchToBestWeapon();
					...
					ClientMessage("Gave "$ gItem.ItemName $" to "$ FrobTarget.FamiliarName);
					return True;
				}
			}
	...
	if (item != None && bDropped && item.IsA('WeaponCombatKnife') && !item.IsA('WeaponToxinBlade'))
	{
		amItem = Ammo(FindInventoryType(Class'AmmoCombatKnife'));
		
		if (amItem != None)
		{
			Weapon(item).PickupAmmoCount = amItem.AmmoAmount;
			amItem.AmmoAmount    = 0;
		}
	}
...
function Bool HasPass(String p, optional String l)
{
	local int i;
	local bool bHasPass;
	
	if (NanoMod.disableAutoFill)
	return false;
...
function Bool AddPass(String p, optional string l)
{
	local int i;
   ...
   // DEUS_EX AMSD DXReduceDamage is slow.  Pass in the result from earlier.
	if (!DamageReduced) 
	{
		if ( (damage > 0) || (ReducedDamageType == 'All') )
		{
			...
			else if (!NanoMod.disableClientFlash)
			{
...
exec function toggleWeaponMod(string type)
{
	local bool setting;
	local string result;
	...
	if (!bCheatsEnabled)
	{
		ClientMessage("Must enable cheats");
	...
	{
		case "ACCURACY":
			noMoreAccuracyMods = !noMoreAccuracyMods;
			setting			   = noMoreAccuracyMods;
		break;
		
		case "CLIP":
			noMoreClipMods = !noMoreClipMods;
			setting		   = noMoreClipMods;
		break;
		
		case "RANGE":
			noMoreRangeMods = !noMoreRangeMods;
			setting		    = noMoreRangeMods;
		break;
		
		case "RECOIL":
			noMoreRecoilMods = !noMoreRecoilMods;
			setting		     = noMoreRecoilMods;
		break;
		
		case "RELOAD":
			noMoreReloadMods = !noMoreReloadMods;
			setting		     = noMoreReloadMods;
		break;
		
		default:
			ClientMessage("Invalid Mod Name");
	...
	ClientMessage("Random Weapon Mod (" $ type $ "):" @ result);
}

exec function tearsPreventionMechanism(bool invincibleCats)
{
	NanoMod.invincibleCats = invincibleCats;
	
	NanoMod.saveConfig();
	NanoMod.applyConfig();
	
	ClientMessage("Invincible Cats:" @ invincibleCats);
}

exec function listModSettings()
{
	NanoMod.addMessage("tearsPreventionMechanism (Invincible Cats):" @ NanoMod.invincibleCats);
	
	if (bCheatsEnabled)
	{
		NanoMod.addMessage("|ntoggleWeaponMod (Random Weapon Mods from bodies):");
		NanoMod.addMessage("Accuracy:" @ !noMoreAccuracyMods);
		NanoMod.addMessage("Clip:" @ !noMoreClipMods);
		NanoMod.addMessage("Range:" @ !noMoreRangeMods);
		NanoMod.addMessage("Reload:" @ !noMoreReloadMods);
		NanoMod.addMessage("Recoil:" @ !noMoreRecoilMods);
	}
	
	NanoMod.showMessage();
}

exec function listItems()
{
	local InformationDevices id;
	local NanoKey nk;
	local AugmentationUpgradeCannister au;
	...
	foreach AllActors(Class'NanoKey', nk)
	{
		if (nk.Owner != None)
		NanoMod.addMessage(nk.Name @ nk.KeyID @ nk.Owner);
		
		else
		NanoMod.addMessage(nk.Name @ nk.KeyID);
	}
	
	foreach AllActors(Class'InformationDevices', id)
	{
		if (!id.bRead && (id.bAddToVault || id.imageClass != None))
			NanoMod.addMessage(String(id.Name));
	}
	
	foreach AllActors(Class'AugmentationCannister', ac)
	{
		if (ac.Owner != Self)
		NanoMod.addMessage(ac.Name @ ac.Owner);
	}
	
	foreach AllActors(Class'AugmentationUpgradeCannister', au)
	{
		if (au.Owner != Self)
		NanoMod.addMessage(au.Name @ au.Owner);
	}
	
	foreach AllActors(Class'WeaponMod', wm)
	{
		if (wm.Owner == None)
		NanoMod.addMessage(String(wm.Name));
	}
	
	NanoMod.showMessage();
}

// a: not for release, sorry!
/*exec function DeusExMachina()
{
	if (versionType < 3)
	return;
	
	bCheatsEnabled = True;
	killAll(Class'ScriptedPawn');
	killAll(Class'SecurityCamera');
	killAll(Class'AutoTurret');
	killAll(Class'AlarmUnit');
	Energy=10000;
	ghost();
	ClientMessage("NanoMod Testing Mode Activated");
}*/
...
function CreateColorThemeManager()
{
	if (ThemeManager == None)
	{
		...
		ThemeManager.AddTheme(Class'ColorThemeHUD_Starlight_Nano');
...
defaultproperties
{
     ...
     InventoryFull="You don't have enough room to pick up the %s"
     ...
     AddedNanoKey="%s added to NanoKey Ring"
