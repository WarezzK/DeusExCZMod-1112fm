	...
	if (bHacking)
	{
      ...
      while (TicksSinceLastHack > TicksPerHack && numHacks > 0)
      {
		...
		if (hackStrength ~= 0.0)
		{
			...
			if (hackPlayer.versionType == 3 && hackPlayer.NanoMod.enableCombatPoints)
			{
				pts = 10;
				
				if (initialHackStrength >= 0.2)
				pts += 10;
				
				if (initialHackStrength >= 0.4)
				pts += 10;
				
				if (initialHackStrength >= 0.6)
				pts += 10;
				
				if (initialHackStrength >= 0.8)
				pts += 10;
				
				hackPlayer.SkillPointsAdd(pts);
			}
