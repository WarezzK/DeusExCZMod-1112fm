...
event Tick(float deltaSeconds)
{
	...
	//a: well, this was a good idea at the time :p
	//too bad it doesn't really work when the targets are so far away that the laser doesn't even reach them, or it becomes too small for human eyes
	/*if (bViewVisible && player != None && player.bCrosshairVisible && player.rootWindow != None)
	{
		cross = DeusExRootWindow(player.rootWindow).hud.cross;

		if (DeusExWeapon(player.inHand).bLasing)
			cross.SetCrosshair(false);
			
		else
			cross.SetCrosshair(true);
	}*/
}
...
event DrawWindow(GC gc)
{
	...
	if ( Player.Level.NetMode == NM_Standalone)	// Only block out screen real estate in single player
	{
		...
		if(DeusExWeapon(Player.inHand) != None) // Check to see if the target aug is doing the zooming, rather than the weapon
		{
			...
			else if (Player.versionType == 3 && player.AugmentationSystem.GetAugLevelValue(class'AugTarget') > -1.0)
			test = false;

			else if(Player.versionType < 3 && desiredFOV == Int(30.000000 + (100.000000 * Player.AugmentationSystem.GetAugLevelValue(class'AugTarget'))))
				test = false;
