...
function ApplyMod(DeusExWeapon weapon)
{
	...
	if (weapon != None)
	{
		modMult = (weapon.Default.BaseAccuracy - 0.5) / weapon.Default.BaseAccuracy / 5;
		
		if (weapon.BaseAccuracy == 0.0)
			weapon.BaseAccuracy    -= modMult;
		else
			weapon.BaseAccuracy    -= (weapon.Default.BaseAccuracy * modMult);
		weapon.ModBaseAccuracy += WeaponModifier;
		
		weapon.BaseAccuracy = Float(String(weapon.BaseAccuracy));
	}
}
