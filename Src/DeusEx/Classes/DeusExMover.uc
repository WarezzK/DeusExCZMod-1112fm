      ...
      while (TicksSinceLastPick > TicksPerPick && numPicks > 0)
      {
		...
		if (lockStrength ~= 0.0)
		{
         ...
         TimeSinceReset = 0.0;
			...
			if (pickPlayer.versionType == 3 && pickPlayer.NanoMod.enableCombatPoints)
			{
				pts = 10;
				
				if (initialLockStrength >= 0.2)
				pts += 10;
				
				if (initialLockStrength >= 0.4)
				pts += 10;
				
				if (initialLockStrength >= 0.6)
				pts += 10;
				
				if (initialLockStrength >= 0.8)
				pts += 10;

				pickPlayer.SkillPointsAdd(pts);
			}
...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	if (!bDone)
	{
		...
		if (frobWith != None)
		{
			...
			else if ((KeyIDNeeded != '') && frobWith.IsA('NanoKeyRing') && (lockStrength > 0.0))
			{
				...
				if (NanoKeyRing(frobWith).HasKey(KeyIDNeeded))
				{
					...
					bOpenIt = False;
					if (bLocked)
						msg = sprintf(msgKeyLocked, NanoKeyRing(frobWith).KeyName(KeyIDNeeded));
					else
						msg = sprintf(msgKeyUnlocked, NanoKeyRing(frobWith).KeyName(KeyIDNeeded));
				}
				else if (bLocked)
				{
					bOpenIt = False;
		...
		{
			//a: uh, please, Justice. Please.
	
			unlocker = player.FindInventoryType(Class'NanoKeyRing');
			
			if (!NanoKeyRing(unlocker).hasKey(KeyIDNeeded))
			{
				if (bPickable)
				unlocker = player.FindInventoryType(Class'Lockpick'); //a: None if not found. None if not pickable. Elegant, huh?
...
defaultproperties
{
     ...
     msgKeyLocked="Locked with the %s NanoKey"
     msgKeyUnlocked="Unlocked with the %s NanoKey"
