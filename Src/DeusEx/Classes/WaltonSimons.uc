...
function bool Facelift(bool bOn)
{
	...
	//a: we need to test this first because of a weird glitch with the hologram
	//if(bOn)
		testMesh = Mesh(DynamicLoadObject("HDTPCharacters.HDTPWaltonSimons", class'Mesh', True));

	if(testMesh == None || !bOn)
	{
		Mesh = Default.Mesh;
		...
		if (testMesh != None && style == STY_Translucent)
			MultiSkins[6] = None;
	}
