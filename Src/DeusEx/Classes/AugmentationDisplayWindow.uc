...
function ConfigurationChanged()
{
	...
	if (DeusExPlayer(GetPlayerPawn()).NanoMod.lowerTargetWindow)
	posDiv = 1.5;
...
function DrawTargetAugmentation(GC gc)
{
	...
	if ( target != None )
	{
		...
		if ((weapon != None) && !weapon.bHandToHand && !bUseOldTarget && !(weapon.bHasScope && !weapon.bZoomed && DeusExGameInfo(Player.Level.Game).noScope) && !DeusExRootWindow(Player.rootWindow).hud.info.isVisible()) //Justice: Remove reticle when (mis)using a scoped weapon //a: nor when reading text
		{
	...
	if (target != None)
	{
		...
		if (ConvertVectorToCoordinates(target.Location, boxCX, boxCY))
		{
			...
			if ((bTargetActive) && (Player.Level.Netmode == NM_Standalone))
			{
				...
				if (targetLevel >= 3 && bTargetWindowActive)
				...
				if (targetLevel >= 0 && bTargetWindowActive)
				{
					if (winZoom != None)
					{
						...
						winZoom.SetViewportLocation(target.Location - mult * v2); //uncomment
						winZoom.SetWatchActor(target); //uncomment
					}
	...
	else if ((bTargetActive) && (Player.Level.NetMode == NM_Standalone) && !DeusExRootWindow(player.rootWindow).hud.info.isVisible())
	{
