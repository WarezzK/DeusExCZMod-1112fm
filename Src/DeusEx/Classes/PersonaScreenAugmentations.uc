...
event InitWindow()
{
	local Texture augBG4;
	
	if (IsA('HUDMedBotAddAugsScreen'))
		augBG4 = Texture(DynamicLoadObject("NanoModUI.Backgrounds.HUDMedbotBackground_4", Class'Texture', True));
	
	else
		augBG4 = Texture(DynamicLoadObject("NanoModUI.Backgrounds.AugmentationsBackground_4", Class'Texture', True));
		
	if(augBG4 != None)
			clientTextures[3] = augBG4;
...
function CreateAugmentationButtons()
{
	...
	local Texture augHeartBG;
	local string strBackground;
	local int    intBackground, tmpHeartLoc;
	local bool   augHeartFound;
	...
	//a: OMG JUST FOR A STUPID AUG ARGHHHH!!!!!!!!! KILL ME....
	if (augHeartLoc == 0 && augHeartFound)
	{
		augHeartLoc = tmpHeartLoc;
		...
		if (IsA('HUDMedBotAddAugsScreen'))
		strBackground = "HUDMedbotBackground";
			
		else
		strBackground = "AugmentationsBackground";

		augHeartBG = Texture(DynamicLoadObject("NanoModUI.Backgrounds." $ strBackground $ "_" $ intBackground $ "_blank" $ augHeartLoc, Class'Texture', True));
	}
	
	//a: UGH this means we replaced the Synthetic Heart with something at the Medbot... Right?
	//a: O*^&T&^G@I&TF!!
	else if (augHeartLoc > 0 && !augHeartFound)
	{
		if (augHeartLoc < 3)
		intBackground = 2;
		...
		if (intBackground == 2 && clientTextures[1] != Texture'DeusExUI.UserInterface.HUDMedbotBackground_2')
			augHeartBG = Texture(DynamicLoadObject("DeusExUI.UserInterface.HUDMedbotBackground_2", Class'Texture', True));
			
		else if (intBackground == 5 && clientTextures[4] != Texture'DeusExUI.UserInterface.HUDMedbotBackground_5')
			augHeartBG = Texture(DynamicLoadObject("DeusExUI.UserInterface.HUDMedbotBackground_5", Class'Texture', True));
	...
	if (augHeartBG != None)
	{
		player.UnloadTexture(clientTextures[(intBackground-1)]);
		clientTextures[(intBackground-1)] = augHeartBG;
		winClient.SetClientTexture((intBackground-1), clientTextures[(intBackground-1)]);
	}
}
