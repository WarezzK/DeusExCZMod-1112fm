...
simulated function PreBeginPlay()
{
	...
	{
		if (player.NanoMod.hasHDTP || IsA('WeaponModAuto'))
		Facelift(true);
	}
}
...
function bool HandlePickupQuery( inventory Item )
{
	...
	if ( Item.Class == Class )
	{
		...
		{
		    //a: just like ammo works
			if (DeusExPickup(item).NumCopies > 1 && NumCopies < MaxCopies && (NumCopies + DeusExPickup(item).NumCopies > MaxCopies))
			DeusExPickup(item).NumCopies = MaxCopies - NumCopies;
