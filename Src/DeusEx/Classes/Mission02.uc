...
function FirstFrame()
{
	...
	if (localURL == "02_NYC_STREET")
	{
		...
		if (!flags.GetBool('M02OsgoodFix'))
		{
			foreach AllActors(class'DeusExMover', M)
			{
				if (M.Name == 'DeusExMover8')
				{
					M.bLocked = True;
					M.bIsDoor = True;
			...
			flags.SetBool('M02OsgoodFix', True,, 3);
		}
	}
	...
	else if (localURL == "02_NYC_SMUG")
	{
		...
		{
			spawn(class'WeaponModAuto', None,, vect(-572.000000, 1717.000000, 245.000000));
			
			if(spawn(class'WeaponWPGrenade', None,, vect(-535.000000, 1700.000000, 243.000000)) != None)
