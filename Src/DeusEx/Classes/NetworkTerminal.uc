...
var ComputerScreenAvailAccounts winAvailAccounts; // Hack Accounts Window, used for email
var ShadowWindow               winHackAccountsShadow;
var ShadowWindow               winAvailAccountsShadow;
...
function ConfigurationChanged()
{
	...
	local float hackAccountsWidth, hackAccountsHeight, availAccountsWidth, availAccountsHeight;
	...
	if (winHackAccounts != None)
	{
		...
		winHackAccountsShadow.ConfigureChild(
			...
			/*winAvailAccountsShadow.ConfigureChild(
			width - hackAccountsWidth + winAvailAccounts.backgroundPosX - shadowOffsetX, 
			hackHeight + 20 + winAvailAccounts.backgroundPosY - shadowOffsetY, 
			winAvailAccounts.backgroundWidth + (shadowOffsetX * 2), 
			winAvailAccounts.backgroundHeight + (shadowOffsetY * 2));*/
	}
	
	if (winAvailAccounts != None)
	{
		winAvailAccounts.QueryPreferredSize(availAccountsWidth, availAccountsHeight);
		winAvailAccounts.ConfigureChild(
			width - availAccountsWidth, hackHeight + 20, 
			availAccountsWidth, availAccountsHeight);

		// Place shadow
			winAvailAccountsShadow.ConfigureChild(
			width - availAccountsWidth + winAvailAccounts.backgroundPosX - shadowOffsetX, 
			hackHeight + 20 + winAvailAccounts.backgroundPosY - shadowOffsetY, 
			winAvailAccounts.backgroundWidth + (shadowOffsetX * 2), 
			winAvailAccounts.backgroundHeight + (shadowOffsetY * 2));
	}
...
function ShowScreen(Class<ComputerUIWindow> newScreen)
{
	...
	if (newScreen != None)
	{
		...
		if (newScreen == Class'ComputerScreenLogin' && winHack != None)
		winHack.SetCompOwner(compOwner);
	}
}
...
function CreateHackWindow()
{
	...
	if ((skillLevel > 0) && (bUsesHackWindow))
	{
		...
		detectionTime  = Default.detectionTime;
...
function CreateAvailAccountsWindow(ComputerScreenLogin CompLogin)
{
	CloseAvailAccountsWindow();
	if ((!bHacked) && (winAvailAccounts == None))
	{
		// First create the shadow window
		winAvailAccountsShadow = ShadowWindow(NewChild(Class'ShadowWindow'));

		winAvailAccounts = ComputerScreenAvailAccounts(NewChild(Class'ComputerScreenAvailAccounts'));
		winAvailAccounts.SetNetworkTerminal(Self);
		winAvailAccounts.CompLogin = CompLogin;
		winAvailAccounts.SetCompOwner(compOwner);
		
		if (winAvailAccounts != None)
		{
		winAvailAccounts.AskParentForReconfigure();
		
		if(Computers(compOwner).NumUsers() < 2)
		CloseAvailAccountsWindow();
		}
	}
	
	if (winAvailAccounts != None && CompLogin != None)
	winAvailAccounts.winCurrentUser.SetText("");
}
...
function CloseAvailAccountsWindow()
{
	if (winAvailAccounts != None)
	{
		winAvailAccounts.Destroy();
		winAvailAccounts = None;

		winAvailAccountsShadow.Destroy();
		winAvailAccountsShadow = None;
	}
}
...
function ComputerHacked()
{
	...
	for (userIndex = 0; userIndex < Computers(compOwner).NumUsers(); userIndex++)
	{
		if (Computers(compOwner).GetAccessLevel(userIndex) <= player.SkillSystem.getSkillLevel(class'SkillComputer') ||
		player.HasPass(Computers(compOwner).GetPassword(userIndex), Computers(compOwner).GetUserName(userIndex)))
			break;
	}
	
	if (compOwner.IsA('ComputersPublic'))
	userIndex = 0;
...
{
	return static.hasSpecialOptions(Computers(compOwner), userName, bCheckActivated);
}

static function bool hasSpecialOptions(Computers oComp, String oName, optional bool bCheckActivated)
{
	local int i;
	local bool bOK;
	...
	bOK = False;
	for (i=0; i<ArrayCount(oComp.specialOptions); i++)
	{
		if (oComp.specialOptions[i].Text != "")
		{
			if ((oComp.specialOptions[i].userName == "") || (Caps(oComp.specialOptions[i].userName) == Caps(oName)))
			{
				...
				if (!((bCheckActivated) && (oComp.specialOptions[i].bAlreadyTriggered)))
				{
					bOK = True;
					break;
				}
