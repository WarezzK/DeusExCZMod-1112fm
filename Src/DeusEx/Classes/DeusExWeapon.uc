...
function PreBeginPlay()
{
	...
	{
		if ((player.NanoMod.hasHDTP && !player.NanoMod.disableHDTPWeapons) || IsA('WeaponMagnum') || IsA('WeaponWPGrenade'))
		Facelift(true);
	}
}
...
simulated function float CalculateAccuracy()
{
	...
	local float tempacc, div, diff, minStandAcc;
   local float weapskill; // so we don't keep looking it up (slower).
	...
	if (player != None)
	{
		...
		if(div != -1.0)
			accuracy += ((player.AugmentationSystem.GetClassLevel(class'AugTarget') + 1) * -0.025);
	...
	{	
		if (Player != None)
		{
			...
			{
				if (accuracy > 0.10)
				minStandAcc = ((accuracy + 0.10) * 0.25);
		...
		tempacc = accuracy;
		if (standingTimer > 0 && (accuracy > minStandAcc))
		{
			...
			// don't go too low
			if ((accuracy < minStandAcc))
				accuracy = minStandAcc;
		}
	}
	...
	//player.ClientMessage(accuracy @ ":" @ 100 - (accuracy * 50) $ "%");
   ...
   if (Level.NetMode != NM_Standalone)
	...
	if(player != None && Default.bHasScope && !bZoomed && !bLasing)
	{
...
simulated function Tick(float deltaTime)
{
   ...
   if ((LockMode == LOCK_Locked) && (SoundTimer > 0.1) && (Role == ROLE_Authority))
   {
	...
	if (player != None)
	{
		...
		//recoil = recoilStrength + GetWeaponSkill() * 1.5;
		recoil = Float(String((recoilStrength + (GetWeaponSkill() * Default.recoilStrength)) * 0.5));
		if (recoil < 0.00)
			recoil = 0.00;

		// simulate recoil while firing
		if (bFiring && IsAnimating() && (AnimSequence == 'Shoot') && (recoil > 0.00))
		{
...
simulated function RefreshScopeDisplay(DeusExPlayer player, bool bInstant, bool bScopeOn)
{
	...
	if (bScopeOn && (player != None))
	{
		...
		{
			if (player.AugmentationSystem.GetAugLevelValue(class'AugTarget') > -1.0)
...
simulated function PlayFiringSound()
{
	...
	else
	{
		...
		{
			if ((IsA('WeaponPlasmaRifle') || IsA('WeaponFlamethrower') || IsA('WeaponPepperGun')) && FireSound == None)
...
simulated function Projectile ProjectileFire(class<projectile> ProjClass, float ProjSpeed, bool bWarn)
{
	...
	if (bLasing || bZoomed) //CyberP: laser had no effect on xbow?
	currentAccuracy = 0.0;
...
simulated function ProcessTraceHit(Actor Other, Vector HitLocation, Vector HitNormal, Vector X, Vector Y, Vector Z)
{
	...
	local float EMPDischargeDrain; //Justice: energy drained by EMP Discharge

	EMPDischargeDrain = 15.0;
	if(AreaOfEffect == AOE_Cone) //Justice: Reduce drain for multihit weapons
		EMPDischargeDrain = EMPDischargeDrain /= 5;
	else if(AreaOfEffect == AOE_Sphere)
		EMPDischargeDrain = EMPDischargeDrain /= 15;
	...
	else if (Other != None)
	{
		...
		EMPDam = -1.0;
		EMPlev = 0;
		if (bHandToHand && (DeusExPlayer(Owner) != None) && DeusEXPlayer(Owner).Energy > 0) //Justice: EMP Discharge only works if you have energy
		{
			mult = DeusExPlayer(Owner).AugmentationSystem.GetAugLevelValue(class'AugMuscle');//(class'AugCombat');
			if (mult == -1.0)
				mult = 1.0;
		...
		if ((Other == Level) || (Other.IsA('Mover')))
		{
			...
			if (EMPDam > 0.0)
			{
				Human(Owner).DrainEnergy(EMPDischargeDrain); //Justice: EMP Discharge uses energy on hit
		...
		else if ((Other != self) && (Other != Owner))
		{
			...
			if ( Role == ROLE_Authority )
			{
				...
				//if (bHandToHand) //Justice: Play spawn effects for ranged weapons
				SelectiveSpawnEffects( HitLocation, HitNormal, Other, HitDamage * mult);
...
simulated function bool UpdateInfo(Object winObject)
{
	...
	//str = String(dmg);
	mod = 1.0 - (2.0 * GetWeaponSkill()); // Thanks CyberP!
	if (mod != 1.0)
	{
	...
	if (HasClipMod())
	{
		...
		str = str @ "(+" $ Max(1, Int(Float(Default.ReloadCount) * 0.1)) $ " x" $ Int(Abs(ModReloadCount) * 10) $ ")";
	...
	{
		str = str @ BuildPercentString(ModReloadTime + (GetWeaponSkill() * Float(String(ReloadTime/Default.ReloadTime))));
	...
	{
		str = str @ BuildPercentString(ModRecoilStrength + (GetWeaponSkill() * 1.9 * recoilStrength/Default.recoilStrength));
		str = str @ "=" @ FormatFloatString(recoilStrength * (1.0 + (GetWeaponSkill() * 1.9)), 0.01);
	}*/
	
	str = FormatFloatString(Default.recoilStrength, 0.01);
	if (HasRecoilMod() || (GetWeaponSkill() < 0.0 && Default.recoilStrength > 0.0))
	{
		mod = Float(String((recoilStrength + (GetWeaponSkill() * Default.recoilStrength)) * 0.5));
		
		if (mod < 0.00)
		mod = 0.00;
		
		str = str @ BuildPercentString(-1.000000 + Float(String((mod / Default.recoilStrength))));
		str = str @ "=" @ FormatFloatString(mod, 0.01);
	}
	...
	else
	{
		...
		if (BaseAccuracy + GetWeaponSkill() <= 0.00)
		mod = Default.BaseAccuracy * 0.5;
		
		else
		mod = (Default.BaseAccuracy - (BaseAccuracy + GetWeaponSkill())) * 0.5;
		
		mod = Float(String(mod));
	...
	if (bCanHaveModBaseAccuracy || bCanHaveModReloadCount || bCanHaveModAccurateRange || bCanHaveModReloadTime || bCanHaveModRecoilStrength || bCanHaveModShotTime)
	{
		winInfo.AddLine();
		...
		if (bCanHaveModReloadCount)
		{
			...
			winInfo.AddModInfo("Clip:", numMods, (numMods == 5));
		}
...
simulated function String FormatFloatString(float value, float precision)
{
	...
	if (precision < 1.0)
	{
		...
		val_len  = Len(String(Int(value)));
		
		value -= Int(value);
		
		str = str $ "." $ Mid(String(value), 1 + val_len, prec_len); 
...
{
	return (Float(String((recoilStrength + (-0.5 * Default.recoilStrength)) * 0.5)) <= 0.00);
