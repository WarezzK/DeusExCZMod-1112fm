...
function FirstFrame()
{
	...
	else if (localURL == "06_HONGKONG_WANCHAI_STREET")
	{
		...
		{
			spawn(class'WeaponModAuto', None,, vect(537.000000, -1245.000000, 1701.000000));
			
			if(spawn(class'WeaponWPGrenade', None,, vect(540.000000, -1264.000000, 1699.000000)) != None)
...
//		}
	...
	else if (localURL == "06_HONGKONG_STORAGE")
	{
		...
		{
			if (dCube.textTag == '06_Datacube24')
				dCube.textTag = '06_Datacube12';
				
			if (dCube.textTag == '06_Datacube25')
			{
				dCube.textTag = '06_Datacube10';
				dCube.setLocation(vect(-392.0, -553.5, 641.5));
			}
		}
		
		if (!flags.GetBool('M06_Failsafe_Placed'))
		{
			foreach AllActors(class'ScientistFemale', SF, 'ScientistFemale')
			{
				key = spawn(class'NanoKey', SF);
				
				if (key != None)
				{
					key.KeyID = 'ncHarrison';
					key.Description = "Nanotech Containment, Harrison's Cabinet";
			...
			{
				if (M.Name == 'DeusExMover18' || M.Name == 'DeusExMover19')
				m.keyIDNeeded = 'ncHarrison';
			}
			
			flags.setBool('M06_Failsafe_Placed', True,, 8);
		}
	}
