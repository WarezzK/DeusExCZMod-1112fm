...
function FirstFrame()
{
	...
	else if (localURL == "04_NYC_NSFHQ")
	{
		...
		foreach AllActors(Class'ControlPanel', cp)
		{
			if (cp.Name == 'ControlPanel2')
			{
				cp.UntriggerEvent[0] = 'Beams2';
		...
		{
			if (lt.Tag == 'LaserTrigger')
				lt.Tag = 'Beams2';
			
			if (lt.Name == 'LaserTrigger44')
			lt.Tag = 'Beams5';
		}
	}
	...
	else if(localURL == "04_NYC_BAR")
	{
		...
		{
			if (dCube.imageClass == Class'DeusEx.Image02_BobPage_ManOfYear')
			dCube.imageClass = Class'Image04_UNATCONotice';
		}
	}
}
