...
{
	local int skillLevel, skillLevelOff, winIconsWidth, winIconsOffWidth;
	
	if (skill != None)
	{
		skillLevel       = skill.GetCurrentLevel();
		skillLevelOff    = 2 - skillLevel;
		winIconsWidth    = (skillLevel + 1) * 6;
		winiconsOffWidth = (skillLevelOff + 1) * 6;
		...
		winLevelIcons.SetWidth(winIconsWidth);
		
		winLevelIconsOff.SetLevel(skillLevelOff);
		winLevelIconsOff.SetPos(229 + (winIconsWidth), 10);
		winLevelIconsOff.SetWidth(winiconsOffWidth);
