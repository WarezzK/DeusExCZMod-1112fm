...
function FirstFrame()
{
	...
	else if (localURL == "15_AREA51_PAGE")
	{
		foreach AllActors(class'Datacube', dCube)
		{
			...
			if (dCube.textTag == '15_Datacube18')
				dCube.imageClass = class'Image15_GrayDisection';
		}
		
		foreach AllActors(Class'ComputerSecurity', S)
		{
			if (S.Name == 'ComputerSecurity1')
			{
				S.userList[0].userName = "LAB 12";
				S.userList[0].Password = "GRAYTEST";
				
				break;
			}
		}
		
		foreach AllActors(Class'KeyPad2', K)
		{
			if (K.Name == 'KeyPad4')
			{
				K.validCode = "2242";
		...
		{
			if (dlt.datalinkTag == 'DL_PageBarks1' || dlt.datalinkTag == 'DL_PageBarks2' || dlt.datalinkTag == 'DL_PageBarks3')
			dlt.bTriggerOnceOnly = True;
		}
