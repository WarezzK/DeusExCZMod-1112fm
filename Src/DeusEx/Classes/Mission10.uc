...
function FirstFrame()
{
	...
	if (localURL == "10_PARIS_METRO")
	{
		...
		{
			if (dCube.textTag == '10_Datacube09')
			{
				dCube.imageClass = Class'DeusEx.Image10_Paris_Metro';
...
function Timer()
{
	...
	{
		if (!flags.GetBool('GotSaferoomCode') && flags.GetBool('GaveCassandraMoney'))
		{
			flags.SetBool('GotSaferoomCode', True,, 11);
			Player.AddNote("Club saferoom code: 1966",, True);
		}
	}
}
