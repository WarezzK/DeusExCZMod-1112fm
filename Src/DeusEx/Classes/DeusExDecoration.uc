...
function PostPostBeginPlay()
{
	...
	{
		if (player.NanoMod.hasHDTP || IsA('SignFloor'))
		Facelift(true);
	}
}
...
simulated function Frag(class<fragment> FragType, vector Momentum, float DSize, int NumFrags) 
{
	...
	//Justice: Clears the decoration from the tracking aug when destroyed
	//trackingAug = AugTarget(DeusExPlayer(GetPlayerPawn()).augmentationSystem.findAugmentation(class'AugTarget'));
	//if(trackingAug != None)
	//	trackingAug.removeTarget(Self);
