...
function failChallenge(string cName, optional bool invisible) //Justice
{
	local name flagName;
...
function DrainEnergy(float drain) //Justice
{
	...
	EnergyDrain += drain;
	EnergyDrainTotal += drain;
}

exec function noScoped() //Justice: For some reason simply changing the variable doesn't cause it to be saved, but using a console command to change it does
{
	if(!DeusExGameInfo(Level.Game).noScope)
	{
		ConsoleCommand("set DeusExGameInfo noScope true");
		ClientMessage("Reticle while using scoped weapons disabled");
	}
	else
	{
		ConsoleCommand("set DeusExGameInfo noScope false");
		ClientMessage("Reticle while using scoped weapons enabled");
	}
}

/*exec function skillsForKills() //Justice
{
	if(!DeusExGameInfo(Level.Game).bNewSkillSystem)
	{
		ConsoleCommand("set DeusExGameInfo bNewSkillSystem true");
		ClientMessage("Shifter Skill System enabled");
	}
	else
	{
		ConsoleCommand("set DeusExGameInfo bNewSkillSystem false");
		ClientMessage("Shifter Skill System disabled");
	}
}*/
