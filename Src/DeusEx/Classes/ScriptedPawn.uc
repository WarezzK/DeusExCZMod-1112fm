...
function PreBeginPlay()
{
	...
	{
		if (player.NanoMod.hasHDTP && (player.NanoMod.enableHDTPHumans || IsA('Robot') || IsA('Animal')))
		Facelift(true);
	}
}
...
function InitializePawn()
{
	if (!bInitialized)
	{
		...
		bInitialized = true;
		SetTimer(3.0, True); //== Random inventory checker
	}
}
...
function InitializeInventory()
{
	...
	if (weaponCount > 1)
	{
		inv = FindInventoryType(Class'WeaponCombatKnife');
	
		if (inv != None && Rand(18) >= 5)
		{
			DeleteInventory(inv);
			inv.Destroy();
			
			inv = FindInventoryType(Class'AmmoCombatKnife');
	...
	if (IsA('Robot') || IsA('Animal') || IsA('GuntherHermann') || IsA('AnnaNavarre') || IsA('WaltonSimons') || IsA('MIB') || IsA('WIB') || IsA('MJ12Commando'))
	return;
	...
	if(testRandom < 4 && (bIsHuman || dxPlayer.combatDifficulty > 4.0))
	{
		Consumables[0] = class'Credits';
		Consumables[1] = class'Medkit';
		Consumables[2] = class'Lockpick';
		Consumables[3] = class'Multitool';
		...
		if(testRandom == 0) //Credits
		{
			inv = spawn(Class'Credits',self);
			if(default.Health >= default.HealthHead)
				Credits(inv).numCredits = Max(default.Health/4, 1 + Rand(default.Health));
			else
				Credits(inv).numCredits = Max(default.HealthHead/4, 1 + Rand(default.HealthHead));
			//log("ScriptedPawn==>Gave Credits in amount of " $Credits(inv).numCredits$ " to " $Self);
		}
		
		else
		inv = spawn(Consumables[testRandom], Self);
...
function Carcass SpawnCarcass()
{
	...
	if ((Health < -100) && !IsA('Robot') && Inventory == None)
	{
...
function bool GenerateRandomInventory()
{
	...
	local bool bHadWeapon, bHadPowerfulWeapon, bHadHeavyWeapon, bRealImportant;
	local DeusExPlayer dxPlayer;
	local int tmp_s, numAvailMods;
	local class<Inventory> availMods[5];
	...
	if (bNPCRandomGiven || IsA('Robot') || IsA('Animal') || IsA('HumanCivilian'))
	return true;
	...
	if (bImportant)
	bRealImportant = (FamiliarName != "Terrorist" && FamiliarName != "MJ12 Commando" && FamiliarName != "MJ12 Troop");
	
	if (IsA('MIB') || IsA('WIB') || IsA('MJ12Commando') || bRealImportant)
	...
	{	
		...
		if(DeusExWeapon(item) != None) //They have a weapon?
		{
			...
			if (!bHadHeavyWeapon)
			bHadHeavyWeapon = (DeusExWeapon(item).GoverningSkill == Class'DeusEx.SkillWeaponHeavy');
				
			if (!bHadPowerfulWeapon && (DeusExWeapon(item).bUnique || item.IsA('WeaponAssaultShotgun') || bHadHeavyWeapon))
				bHadPowerfulWeapon = True;
		}
	...
	if (!dxPlayer.noMoreAccuracyMods)
	{
		availMods[numAvailMods] = Class'WeaponModAccuracy';
		
		numAvailMods++;
	}
	
	if (!dxPlayer.noMoreClipMods)
	{
		availMods[numAvailMods] = Class'WeaponModClip';
		
		numAvailMods++;
	}
	
	if (!dxPlayer.noMoreRangeMods)
	{
		availMods[numAvailMods] = Class'WeaponModRange';
		
		numAvailMods++;
	}
	
	if (!dxPlayer.noMoreReloadMods)
	{
		availMods[numAvailMods] = Class'WeaponModReload';
		
		numAvailMods++;
	}
	
	if (!dxPlayer.noMoreRecoilMods)
	{
		availMods[numAvailMods] = Class'WeaponModRecoil';
		
		numAvailMods++;
	}
	
	if (IsA('AnnaNavarre') || IsA('GuntherHermann') || IsA('WaltonSimons'))
	item = spawn(Class'WeaponModAuto',self);
	
	else if (numAvailMods > 0)
	{
		if (IsA('MJ12Commando') || bHadHeavyWeapon)
		tmp_s = numAvailMods * 40;
		
		if (IsA('MIB') || IsA('WIB'))
		{
			tmp_s = numAvailMods * 24;
			
			if (bHadHeavyWeapon)
			tmp_s = numAvailMods * 10;
		}
		...
		if (tmp_s > 0)
		{
			if (Rand(tmp_s) < numAvailMods * 10)
				item  = Spawn(availMods[Rand(numAvailMods)], Self);
		}
	}
	...
	if (item != None)
	{
		...
		if (bRealImportant)
		WeaponMod(item).bFromRandom = True;
	}
	
	if (IsA('GuntherHermann') || IsA('AnnaNavarre') || IsA('WaltonSimons'))
	return true;
	
	item = None;

	if(bHadWeapon && IsA('HumanMilitary') && ((bHadPowerfulWeapon && Rand(18) < 5) || (!bHadPowerfulWeapon && Rand(30) < 5)))
	{
		if (IsA('MIB') || IsA('WIB') || IsA('MJ12Commando'))
		tmp_s = 6;
		...
		switch(Rand(tmp_s))
		{
			...
			case 6:
					...
					WeaponGrenade(item).bFromRandom = True;
				break;
		}
	}
                            ...
                            Vector offset, Name damageType)
{
	...
	if (damageType == 'TearGas' || damageType == 'EMP' || damageType == 'NanoVirus' || damageType == 'CigSmoke')
		actualDamage = 0;
                                      ...
                                      bool bHatePrimary, bool bHateSecondary)
{
	...
	if(damageType == 'CigSmoke') //Justice: Special case for cigarette smoke
		return false;
...
function EHitLocation HandleDamage(int actualDamage, Vector hitLocation, Vector offset, name damageType)
{
		...
		if (offset.z > headOffsetZ)		// head
		{
			...
			if ((Abs(offset.x) < headOffsetY) || (Abs(offset.y) < headOffsetY))
			{
				...
				if ((damageType == 'Stunned') || (damageType == 'KnockedOut'))
					HealthHead -= actualDamage * 4; //a: wth NOT?
				else
	...
	else if(damageType == 'CigSmoke')
		hitPos = HITLOC_TorsoFront; //Justice: Smoking hits us in the lungs (and this animation fits best)
                        ...
                        bool bPlayAnim)
{
	...
	if (!bPlayAnim || (actualDamage <= 0 && damageType != 'CigSmoke')) //Justice: Play the hit animation for cigsmoke
		hitPos = HITLOC_None;
	...
	if (bCanBleed)
		if ((damageType != 'Stunned') && (damageType != 'TearGas') && (damageType != 'HalonGas') &&
		    ...
		    (damageType != 'Poison') && (damageType != 'PoisonEffect') && (damageType != 'CigSmoke')) //Justice: Don't bleed from cigarette smoke
...
function PlayTakeHitSound(int Damage, name damageType, int Mult)
{
	...
	if (Level.TimeSeconds - LastPainSound < 1.0) //Justice: A longer time between pain sounds to cut down on the "oof oof oof oof oof" factor
		return;
	if (Damage <= 0 && damageType != 'CigSmoke') //Justice: Cigsmoke makes you cough even though it doesn't do damage
		return;
	...
	if(hitSound1 == sound'MalePainSmall' || hitSound1 == sound'FemalePainSmall')
	{
		...
		else if(damageType == 'PoisonGas' || damageType == 'CigSmoke')
		{
			if(!bIsFemale)
...
function bool SwitchToBestWeapon()
{
	...
	while (inv != None)
	{
		...
		if (curWeapon != None)
		{
			...
			if (curWeapon.IsA('WeaponGrenade') && WeaponGrenade(curWeapon).bFromRandom)
			bValid = false;
...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	if (DeusExPlayer(Frobber) != None)
	{
		...
		else if(reactionLevel < 1.0 && AICanSee(Frobber, ComputeActorVisibility(Frobber), true, true, true, true) <= 0 && !canConverseWithPlayer(DeusExPlayer(Frobber)))
		{
			...
			if (item.IsA('NanoKey'))
			{
				if (DeusExPlayer(Frobber).PickupNanoKey(NanoKey(item)))
...
function SkillsForKills(pawn Killer, name damageType, vector HitLocation)
{
	...
	if(DeusExGameInfo(Level.Game) != None)
		skillon = player.NanoMod.enableCombatPoints; //Makes sure the skillpoint system is enabled
	...
	if( (player == DeusExPlayer(Killer) || robokill) && skillon && player != None)
	{
		...
		if(IsA('Rat') || IsA('Bird') || IsA('Fly') || IsA('Fishes') || IsA('Cat'))  //Unless they're a pest or of little consequence
				skillpt = 0;
		...
		if(skillpt != 0 && player.NanoMod.enableCombatPoints) //Now if the level is 0 there's really no point to do any of the rest
		{
...
state Burning
{
	function ReactToInjury(Pawn instigatedBy, Name damageType, EHitLocation hitPos)
	{
		...
		if (health > 0)
		{
			...
			if ((damageType != 'TearGas') && (damageType != 'HalonGas') && (damageType != 'Stunned') && (damageType != 'CigSmoke'))
				GotoDisabledState(damageType, hitPos);
		}
	}
...
state RubbingEyes
{
	...
	function ReactToInjury(Pawn instigatedBy, Name damageType, EHitLocation hitPos)
	{
		if ((damageType != 'TearGas') && (damageType != 'HalonGas') && (damageType != 'Stunned') && (damageType != 'CigSmoke')) //Justice: Don't get multihit by cig smoke
