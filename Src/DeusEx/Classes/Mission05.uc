...
function FirstFrame()
{
	...
	if (localURL == "05_NYC_UNATCOMJ12LAB")
	{
		...
		if (!flags.GetBool('MS_InventoryRemoved'))
		{
			...
			if (Player.Inventory != None)
			{
				...
				foreach AllActors(class'SpawnPoint', SP, 'player_inv')
				{
					...
					//a: not anymore, since we can't easily hack the terminals now.
					else
					{
		...
		{
			spawn(class'AmmoPlasma', None,, vect(-8583.000000, 949.000000, 14.000000));
			
			if(spawn(class'WeaponWPGrenade', None,, vect(-8285.000000, 970.000000, 20.000000)) != None)
				flags.SetBool('M05_WPGrenade_Placed',True);
		}

		foreach AllActors(class'DeusExMover', M)
		{
			if (M.Name == 'DeusExMover33' || M.Name == 'DeusExMover35')
				M.KeyIDNeeded = 'MIBCabinet';
		}
	}
	else if (localURL == "05_NYC_UNATCOHQ")
	{
		...
		if(!flags.GetBool('M05_JM_Locked'))
		{
			foreach AllActors(class'DeusExMover', M)
			{
				...
				{
					M.bLocked = True;
					M.KeyIDNeeded = 'ManderleyOfficeDoor';
					
					break;
				}
			}
			
			flags.SetBool('M05_JM_Locked',True);
		}
	}
